...
const FAQS: FAQItem[] = [
  ...
  {
    question: "What does Glen read?",
    answer:
      "Agent sessions from whichever clients your team runs, plus Slack, GitHub, tickets, calls, and docs. The integrations sync once and only one person needs to set them up. Glen reads public material only, such as public Slack channels and shared team transcripts.",
  },
  {
    question: "Which agents and clients does it work with?",
    answer:
      "Any MCP client: Claude Code, Cursor, Codex, and any agent you built yourself. There is also a command line tool, and both routes give full access to everything Glen does. The web app, the PR reviewer, and the Slack bot come with your account and need no setup at all.",
  },
  {
...
interface GlenFAQProps {
  onOpenWaitlist?: () => void;
}

export function GlenFAQ({ onOpenWaitlist }: GlenFAQProps) {
      ...
      <section>
        <div className="mx-auto w-full max-w-[85rem] px-5 md:px-10">
          <h2 className="text-center text-base md:text-lg">
            What teams usually ask before connecting Glen. Still have a question?{" "}
            <button
              type="button"
